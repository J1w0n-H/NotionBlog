  ...
  },
  {
    ...
    cards: [
      ...
      },
      {
        ...
        body: "<p>Investigated how downstream workarounds mask upstream library bugs, leaving risks unindexed by CVE trackers. Started this independent research during a graduate systems course (CMSC818) to address a core gap: datasets like ARVO lack supply chain attribution labels. Designed a two-stage pipeline combining structural patch-location signals with LLM semantic analysis to detect misalignments between crash paths and defensive code, filtering <strong>23 bypass candidates across 517 cases</strong>.</p><p>Following a submission to WOOT '26, peer reviews validated the premise as a <em>\"compelling and unique problem space\"</em> and highly valued the localized approach for practical industry adoption. However, they noted the risk of LLM inference misidentifying API misuse as library defects without exhaustive manual ground-truth verification.</p><p>Embracing this, I shifted away from heavy LLM reliance to refine <strong>lightweight structural signals</strong>—specifically weighing raw build artifacts and stack-trace location weighting—to position the tool as a practical, deterministic <strong>risk screener</strong>. While final validation requires a comprehensive manual ground-truth review and large-scale API re-experiments, I froze further extension to align with my May 2026 graduation timeline, successfully proving the engineering feasibility of a localized supply chain screener.</p>",
